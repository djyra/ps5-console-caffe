...
class TimeDialog(tkinter.simpledialog.Dialog):
    ...
    def body(self, sony):
        ...
        self.two_button_am = tk.Button(meso,
                                    text='2h \n 500 RSD',
                                    width=5,
                                    height=3,
                                    fg='white',
        ...
        self.three_button_am = tk.Button(meso,
                                      text='3h \n 600 RSD', 
                                      width=5,
                                      height=3,
                                      fg='white',
        ...
        self.one_button = tk.Button(meso, 
                                    text='1h \n 350 RSD', 
                                    width=5,
                                    height=3,
                                    fg='white',
        ...
        self.two_button = tk.Button(meso,
                                    text='2h \n 600 RSD',
                                    width=5,
                                    height=3,
                                    fg='white',
        ...
        self.three_button = tk.Button(meso,
                                      text='3h \n 800 RSD', 
                                      width=5,
                                      height=3,
                                      fg='white',
...
class ReceiptDialog(tkinter.simpledialog.Dialog): 
    ...
    def body(self, menu):
        ...
        meso = tk.Label(self)
        meso.pack()
        artikel = tk.Label(meso, text='ARTIKAL')
        artikel.grid()
        cena = tk.Label(meso, text='CENA')
        cena.grid(row=0, column=1, padx=5, pady=5)
        kolicina = tk.Label(meso, text='KOL.')
        kolicina.grid(row=0, column=2) 
        ukupno = tk.Label(meso, text='UKUPNO')
        ukupno.grid(row=0, column=3)

        for i, s in enumerate(summary, 1):
            print(i)
            for idx, value in enumerate(s.values()):
                val_lab = tk.Label(meso, text=value)
                if i == 0:
                   val_lab['anchor'] = 'w'
        ...
        start = len(summary)
        for i in sony_price:
            sony_lab = tk.Label(meso, text='SONY')
            sony_lab.grid(row=start+1, column=0)
            sony_price = tk.Label(meso, text=i)
            sony_price.grid(row=start+1, column=3)
            start += 1


        total = sum([int(x['sum']) for x in summary]) + sum(self.menu.sony.price)
        total_lab = tk.Label(meso, text=total, font='-weight bold -size 15')
        total_lab.grid(column=3)

        pay_btn = tk.Button(meso, text='NAPLATI', command=self.pay, background='green', foreground='white')
